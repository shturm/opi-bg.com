	...
	function calculate() {
		...
		if(!empty($cart['Product'])) {
			...
			foreach($cart['Product'] as $item) {
				...
				$discount_text = '';
				$subtotal = 0;
				if($item['in_action'] == true) {
					// debug($item);
					$discount = 0;
					$discount_text = 'Aкция';
					$price_1 = $item['action_price'];
					$price_6 = $item['action_price'];
					$price_12 = $item['action_price'];
					$price_24 = $item['action_price'];
				} else {
					$discount = $item['discount'];
					if($item['discount'] > 0) $discount_text = 'Продуктова';
					$price_1 = $item['price'];
					$price_6 = $item['price'];
					$price_12 = $item['price'];
					$price_24 = $item['price'];
			...
			/* 	debug($discount);
				...
				if($this->Auth->user()) {
					...
					switch($user['User']['role']) {
						case 'vendor':
							// vendor doesn't have any discounts, except if product is in action.
							// If in action - prices are not overrided. Else - override them
							$discount = 0;
							if(!$item['in_action']) {
								$discount_text = 'Търговска';
							...
							break;
						case 'shop':
							// shop overrides product discount and action_price
							$discount = $user['User']['discount'];
							$discount_text = 'Магазинерска';
							$price_1 = $item['price'];
							$price_6 = $item['price'];
							$price_12 = $item['price'];
							$price_24 = $item['price'];
						...
						case 'user':
							...
							if($user['User']['discount'] > 0) {
								if($item['in_action']) {
									// discount is already set in the default values
									// when in action, the product cant be applied with any discounts
								}
								if($item['discount'] > 0) {
									// discount is already set in the default values
									// when user and product have discounts - the product discount applies
								} else {
									$discount_text = 'Индивидуална';
									$discount = $user['User']['discount'];
								}
							}										
							break;
				...
				/* Calculate the price for the product applying quantity and discount logic. 
				   Used for total sum
				*/
				$current_price = $price_1 - (($price_1/100) * $discount);
				if($item['type'] == 'lacquer') {
					$price_24 = $price_12;
					if($count_lacquers < 6) {
						$current_price = $price_1 - (($price_1/100) * $discount);
						$total += $subtotal = $price_1*$item['cart_quantity'] - (($price_1*$item['cart_quantity'])/100)*$discount;
					} else if ($count_lacquers >=6 && $count_lacquers < 12) {
						$current_price = $price_6 - (($price_6/100) * $discount);
						$total += $subtotal = $price_6*$item['cart_quantity'] - (($price_6*$item['cart_quantity'])/100)*$discount;
					} else if ($count_lacquers >= 12) {
						$current_price = $price_12 - (($price_12/100) * $discount);
						$total += $subtotal = $price_12*$item['cart_quantity'] - (($price_12*$item['cart_quantity'])/100)*$discount;
					}
					...
					if($count_paints < 12) {
						$current_price = $price_1 - (($price_1/100) * $discount);
						$total += $subtotal = $price_1*$item['cart_quantity'] - (($price_1*$item['cart_quantity'])/100)*$discount;
					} else if ($count_paints >= 12 && $count_paints < 24) {
						$current_price = $price_12 - (($price_12/100) * $discount);
						$total += $subtotal = $price_12*$item['cart_quantity'] - (($price_12*$item['cart_quantity'])/100)*$discount;
					} else if ($count_paints >= 24) {
						$current_price = $price_24 - (($price_24/100) * $discount);
						$total += $subtotal = $price_24*$item['cart_quantity'] - (($price_24*$item['cart_quantity'])/100)*$discount;					
					}
				} else {
					// debug($item['code'] . ' is not a lacquer');
					$price_24 = $price_12;
					$current_price = $price_1 - (($price_1/100) * $discount);
					$total += $subtotal = $price_1*$item['cart_quantity'] - (($price_1*$item['cart_quantity'])/100)*$discount;
				}
				// debug($item['name'] . '</br>' . 'price_1: ' . $price_1 . '</br>' . 'quantity: ' . $item['cart_quantity'] . '</br>' . 'subtotal: ' . $item['subtotal'] . '</br>' . 'type: ' . $item['type']);
