 ...
 */
	...
	/*function orderUp($model, $id = null) {
		if(!$id) return 'bad id supplied'; // bad id supplied
		$current = $model->findById($id);
		// debug($current[$model->alias]);
		if(empty($current)) return 'no record with this id'; // no record with this id
		$upper = $model->findByRght($current[$model->alias]['lft']-1);
		if(empty($upper)) return 'no upper ordered node. current is the top ordered on this level'; // no upper ordered node. $current is the top ordered on this level
		
		// get children (and grandchildren) if any; TODO: use $this->settings[$Model->alias]['left'] & right
		$current_children = $model->find('all',array(
			'conditions' => array(
				$model->alias . '.lft >' => $current[$model->alias]['lft'],
				$model->alias . '.rght <' => $current[$model->alias]['rght']
			),
			'order' => $model->alias . '.lft ASC'
		));
		$upper_children = $model->find('all',array(
			'conditions' => array(
				$model->alias . '.lft >' => $upper[$model->alias]['lft'],
				$model->alias . '.rght <' => $upper[$model->alias]['rght']
			),
			'order' => $model->alias . '.lft ASC'
		));
		
		// set the "Right minus Left" values for later use when calculating the Right value when Left is set with new value
		$current[$model->alias]['r-l'] = $current[$model->alias]['rght'] - $current[$model->alias]['lft'];
		$upper[$model->alias]['r-l'] = $upper[$model->alias]['rght'] - $upper[$model->alias]['lft'];
		if(!empty($current_children)) {
			foreach($current_children as $key => $cc) {
				$current_children[$key][$model->alias]['r-l'] = $cc[$model->alias]['rght'] - $cc[$model->alias]['lft'];
				$current_children[$key][$model->alias]['Cl-Pl'] = $cc[$model->alias]['lft'] - $current[$model->alias]['lft']; //Parent.Left - Child/GrandChild.Left - to preserve the structure
			}
		}
		if(!empty($upper_children)) {
			foreach($upper_children as $key => $uc) {
				$upper_children[$key][$model->alias]['r-l'] = $uc[$model->alias]['rght'] - $uc[$model->alias]['lft'];
				$upper_children[$key][$model->alias]['Cl-Pl'] = $uc[$model->alias]['lft'] - $upper[$model->alias]['lft']; //Parent.Left - Child/GrandChild.Left - to preserve the structure
			}
		}
		
		// update the left and right values now
		$cur_lft = $current[$model->alias]['lft']; // buffer to swap later
		// update for the top level elements
		$current[$model->alias]['lft'] = $upper[$model->alias]['lft'];
		$current[$model->alias]['rght'] = $current[$model->alias]['lft'] + $current[$model->alias]['r-l'];
		$upper[$model->alias]['lft'] = $cur_lft;
		$upper[$model->alias]['rght'] = $upper[$model->alias]['lft'] + $upper[$model->alias]['r-l'];
		// now update their children if they have any
		if(!empty($current_children)) {
			foreach($current_children as $key => $cc) {
				$current_children[$key][$model->alias]['lft'] = $current_children[$key][$model->alias]['Cl-Pl'] + $current[$model->alias]['lft'];
				$current_children[$key][$model->alias]['rght'] = $current_children[$key][$model->alias]['lft'] + $current_children[$key][$model->alias]['r-l'];
			}
		}
		if(!empty($upper_children)) {
			foreach($upper_children as $key => $uc) {
				$upper_children[$key][$model->alias]['lft'] = $upper_children[$key][$model->alias]['Cl-Pl'] + $upper[$model->alias]['lft'];
				$upper_children[$key][$model->alias]['rght'] = $upper_children[$key][$model->alias]['lft'] + $upper_children[$key][$model->alias]['r-l'];
			}
		}
		// save that whole bunch
		$parents = $current + $upper;
		$children = $current_children + $upper_children;
		$data = $parents + $children;
		debug($data); die();
		if ($model->save($data, array('callbacks' => false) )) {
			return true;
		} else return 'couldnt save the whole bunch';
    ...
    function orderUp($model, $id = null) {
	 ...
	 $upper[$model->alias]['lft'] = $current[$model->alias]['lft'];
	 $upper[$model->alias]['rght'] = _reindex($model,$current_children,$upper[$model->alias]['lft']+1);
	 
	 $current[$model->alias]['lft'] = $upper[$model->alias]['rght']+1;
	 $current[$model->alias]['rght'] = _reindex($model,$upper_children,$current[$model->alias]['lft']+1);
	 
	 //Collect and save data (Debug now)
	 $parents = $current + $upper;
	 $children = $current_children + $upper_children;
	 $data = $parents + $children;
	 debug($data); die();
	 ...
	 /*if ($model->save($data, array('callbacks' => false) ))
	    ...
	    return 'couldnt save the whole bunch';*/
    }
	
    function _reindex($model,$children,$index)
    {
     //
     $count = $index;
     ...
     foreach($children as $child)
      {
       ...
       //get the children of the child if any 
       $child_children = $model->find('all',array(
			'conditions' => array(
				$model->alias . '.lft >' => $child[$model->alias]['lft'],
				$model->alias . '.rght <' => $child[$model->alias]['rght']
			),
			'order' => $model->alias . '.lft ASC'
		));
	   
	   //set the left its increment before the call so no increment needed
	   $child[$model->alias]['lft'] = $count;
	   
	   //Set the right index
	   if(!empty($child_children)) 
        $count = $child[$model->alias]['rght'] = $this->_reindex($model,$child_children,$count+1);
       else 
       	$count = $child[$model-alias]['rght'] = $count+1;
      }
     
     //Increment to get the right count of the right
     return $count+1;
    }
}
